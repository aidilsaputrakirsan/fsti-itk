...
watch(search, debounce(() => {
    router.get(route('profil.tenaga-kependidikan'), { search: search.value }, { preserveState: true, replace: true, onFinish: () => AOS.refresh() });
}, 400));
            ...
            <div class="container relative z-10 mx-auto px-4 md:px-6 max-w-7xl">
                ...
                <div v-if="tendik.data.length > 0">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 xl:gap-8">
                        <div v-for="(person, index) in tendik.data" :key="person.id" class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] hover:-translate-y-1.5 transition-all duration-500 flex flex-col group relative" data-aos="fade-up" :data-aos-delay="(index % 4) * 50">
