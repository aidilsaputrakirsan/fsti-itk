...
watch([search, prodi], debounce(() => {
    router.get(route('profil.dosen'), { search: search.value, prodi: prodi.value }, { preserveState: true, replace: true, onFinish: () => AOS.refresh() });
}, 400));
        ...
        <div class="relative bg-slate-50 py-16 md:py-24 font-public-sans min-h-screen overflow-hidden">
            ...
            <div class="container relative z-10 mx-auto px-4 md:px-6 max-w-7xl">
                ...
                <Teleport to="body">
                    <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                        <div v-if="isOpen" id="teleported-dropdown" :style="dropdownStyle" class="z-[9999] bg-white rounded-xl shadow-lg border border-gray-100 py-2 font-public-sans">
                            ...
                            <a v-if="prodiList" v-for="p in prodiList" :key="p" @click="selectProdi(p)" class="block px-5 py-3 text-gray-700 font-medium hover:bg-primary/5 hover:text-primary cursor-pointer transition-colors" :class="{'bg-primary/5 text-primary': prodi === p}">{{ p }}</a>
                        </div>
                    </transition>
                </Teleport>
                ...
                <div v-if="isFiltering" class="mb-8 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4" data-aos="fade-up">
                    ...
                    <button @click="search = ''; selectProdi('')" class="px-5 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-bold rounded-xl transition-colors self-start sm:self-auto">Reset Filter</button>
                </div>

                <div v-if="groupedDosen && Object.keys(groupedDosen).length > 0" class="space-y-16">
                    <div v-for="(lecturers, prodiName) in groupedDosen" :key="prodiName" data-aos="fade-up">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 xl:gap-8">
                            <div v-for="(person, index) in lecturers" :key="person.id" class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] hover:-translate-y-1.5 transition-all duration-500 flex flex-col group relative">
