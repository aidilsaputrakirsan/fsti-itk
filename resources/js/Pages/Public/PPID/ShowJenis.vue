        ...
        <section class="bg-white py-12 md:py-20 min-h-screen font-inter">
            <div class="container mx-auto px-6 lg:px-8 max-w-6xl">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 mb-10 relative overflow-hidden">
                    ...
                    <div>
                        <h2 class="text-2xl md:text-3xl font-kulim-park-bold text-[#133E87] mb-2">Katalog Dokumen {{ jenis_informasi }}</h2>
                        <p class="text-gray-900 font-inter-semibold text-sm md:text-[15px]">Informasi publik tingkat fakultas yang masuk dalam kategori {{ jenis_informasi }}.</p>
                    </div>
                </div>
                ...
                <div v-if="kategoris && kategoris.length > 0" class="space-y-10">
                    <div v-for="kat in kategoris" :key="kat.id" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div class="bg-[#CBDCEB]/40 px-6 py-5 border-b border-[#CBDCEB]/80 flex items-center">
                            <div class="mr-3 text-[#4682A9] flex-shrink-0">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
                                </svg>
                            </div>
                            <h3 class="text-[17px] md:text-lg font-kulim-park-bold text-[#133E87] tracking-tight leading-snug">
                                {{ kat.nama_kategori }}
                            </h3>
                        </div>

                        <div class="divide-y divide-gray-100">
                            <div v-for="doc in kat.dokumen" :key="doc.id" class="group flex flex-col md:flex-row md:items-center justify-between p-5 md:px-6 hover:bg-[#CBDCEB]/10 transition-all duration-300 relative">
                                ...
                                <a :href="doc.file_url" target="_blank" class="inline-flex items-center justify-center px-5 py-2 bg-white text-gray-900 hover:text-white hover:bg-[#133E87] border border-gray-200 hover:border-[#133E87] font-inter-semibold text-sm rounded-lg transition-all duration-300 flex-shrink-0 shadow-sm">
                ...
                <div v-else class="text-center py-20 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
                    <p class="text-gray-500 font-inter-semibold">Belum ada kategori informasi untuk klasifikasi ini.</p>
                </div>
            </div>
        </section>
    </PublicLayout>
