...
const props = defineProps<{ pengabdians: any; stats: { total: number }; filters: any; years: string[]; prodis: { id: number; name: string }[]; }>();
...
const isYearOpen = ref(false); const isProdiOpen = ref(false);
const yearBtnRef = ref<HTMLButtonElement | null>(null); const prodiBtnRef = ref<HTMLButtonElement | null>(null);
const yearDropdownStyle = ref({}); const prodiDropdownStyle = ref({});

const selectedProdiName = computed(() => {
    if (!selectedProdi.value) return '';
    const p = props.prodis.find((x) => x.id == selectedProdi.value); return p ? p.name : '';
});
...
const applyFilters = () => { router.get(route('pengabdian.index'), { search: search.value, year: selectedYear.value, prodi: selectedProdi.value }, { preserveState: true, replace: true }); };
const resetSearch = () => { search.value = ''; };

const currentPage = computed<number>(() => props.pengabdians?.current_page || 1);
const totalPages = computed<number>(() => props.pengabdians?.last_page || 1);
const showingFrom = computed<number>(() => props.pengabdians?.from || 0);
const showingTo = computed<number>(() => props.pengabdians?.to || 0);
const totalPengabdians = computed<number>(() => props.pengabdians?.total || 0);
...
const changePage = (page: number | string) => {
    if (typeof page === 'number' && page >= 1 && page <= totalPages.value) {
        router.get(route('pengabdian.index'), { search: search.value, year: selectedYear.value, prodi: selectedProdi.value, page: page }, { preserveState: true, replace: true, onFinish: () => { window.scrollTo({ top: 450, behavior: 'smooth' }); } });
    }
};
            ...
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
                ...
                <div class="relative z-20 -mt-16 mx-4 md:mx-8 mb-16 bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4" data-aos="fade-down">
                    ...
                    <div class="relative md:w-72">
                        <button ref="prodiBtnRef" @click="toggleDropdown('prodi')" class="w-full pl-12 pr-10 py-3.5 border border-slate-200 rounded-xl bg-slate-50 hover:bg-white text-slate-800 font-medium flex items-center justify-between text-left transition-colors focus:ring-2 focus:ring-primary focus:border-primary">
                            <span class="truncate">{{ selectedProdiName || 'Semua Program Studi' }}</span>
                            <ChevronDown class="w-5 h-5 text-primary/60" :class="{'rotate-180': isProdiOpen}" />
                        </button>
                        ...
                        <table class="w-full min-w-[900px] text-left border-collapse">
                            <thead>
                                <tr class="bg-gradient-to-r from-primary to-primary-hover border-b border-primary/20">
                                    ...
                                    <th class="px-6 py-5 font-bold text-sm text-white tracking-wider w-[20%]">Nama Dosen Pelaksana</th>
                                    <th class="px-6 py-5 font-bold text-sm text-white tracking-wider w-[45%]">Judul Pengabdian</th>
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <tr v-for="(item, index) in pengabdians.data" :key="item.id" class="hover:bg-slate-50/80 transition-colors duration-200 group">
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        <span class="px-3 py-1.5 bg-slate-50 border border-slate-100 text-slate-600 rounded-lg font-semibold group-hover:bg-blue-50 group-hover:text-primary">{{ item.study_program?.name || 'FSTI' }}</span>
                                    </td>
                ...
                <div v-if="totalPages > 1" class="flex flex-col md:flex-row items-center justify-between gap-6 bg-white py-4 px-6 md:px-10 rounded-full shadow-sm border border-slate-100" data-aos="fade-in">
                    <p class="text-sm font-medium text-slate-500 text-center md:text-left">
                        Menampilkan <span class="text-primary font-bold">{{ showingFrom }}</span> - <span class="text-primary font-bold">{{ showingTo }}</span> dari <span class="text-primary font-bold">{{ totalPengabdians }}</span> Data
                    </p>
                    <div class="flex flex-wrap justify-center items-center gap-2">
                        <button @click="prevPage()" :disabled="currentPage === 1" class="min-w-[2.5rem] h-10 px-4 flex items-center justify-center text-sm font-bold rounded-full" :class="currentPage === 1 ? 'text-slate-300 bg-slate-50/50 cursor-not-allowed' : 'text-slate-600 hover:bg-slate-100 hover:text-primary'" v-html="'&laquo; Previous'"></button>
