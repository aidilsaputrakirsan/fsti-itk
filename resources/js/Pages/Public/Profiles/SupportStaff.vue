...
watch(search, debounce(() => {
    router.get(route('profiles.support-staff'), { search: search.value }, { preserveState: true, replace: true, onFinish: () => AOS.refresh() });
}, 400));
    ...
    <PublicLayout>
            ...
            <div class="container relative z-10 mx-auto px-4 md:px-6 max-w-7xl">
                <div class="relative max-w-3xl mx-auto mb-14 rounded-[2rem] bg-white p-8 md:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100 overflow-hidden text-center" data-aos="fade-up">
                    ...
                    <div class="relative z-10 flex flex-col items-center">
                        ...
                        <p class="text-gray-600 font-medium text-[15px] md:text-[16px] max-w-xl mx-auto leading-relaxed">Kenali lebih dekat staf dan tenaga kependidikan Fakultas Sains dan Teknologi Informasi ITK yang siap mendukung layanan akademik dan administratif Anda.</p>
                    </div>
                </div>

                <div class="mb-12 max-w-2xl mx-auto relative z-20" data-aos="fade-down">
                    <div class="relative w-full shadow-sm rounded-xl">
                        <input type="text" placeholder="Cari nama atau jabatan tenaga kependidikan..." v-model="search" class="w-full pl-14 pr-12 py-4 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary bg-white text-gray-800 font-medium transition-all text-base" />
                        <Search class="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                        <button v-if="search" @click="search = ''" class="absolute right-4 top-1/2 -translate-y-1/2 p-1.5 text-gray-400 hover:text-gray-600 transition-colors rounded-lg hover:bg-gray-100"><X class="w-5 h-5" /></button>
                    </div>
                </div>
                ...
                <div v-if="staff.data.length > 0">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 xl:gap-8">
                        <div v-for="(person, index) in staff.data" :key="person.id" class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] hover:-translate-y-1.5 transition-all duration-500 flex flex-col group relative" data-aos="fade-up" :data-aos-delay="(index % 4) * 50">
                    ...
                    <div v-if="staff.links && staff.links.length > 3" class="mt-16 flex justify-center items-center flex-wrap gap-2 pb-8" data-aos="fade-up">
                ...
                <div v-else class="bg-white border border-gray-100 rounded-3xl p-16 text-center shadow-sm" data-aos="zoom-in">
                    ...
                    <p class="mt-2 text-gray-500 font-medium max-w-md mx-auto">Tenaga kependidikan dengan kata kunci "{{ search }}" tidak ditemukan.</p>
                </div>
            </div>
        </div>
        ...
        <transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition ease-in duration-200" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
            <div v-if="isModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 sm:p-6 font-public-sans" @click.self="closeModal">
                <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-h-[90vh] overflow-hidden flex flex-col transform transition-all border border-white/20" :class="hasDetailedInfo ? 'max-w-5xl' : 'max-w-[450px]'" v-if="selectedPerson">
                    ...
                    <div class="bg-gradient-to-r from-primary to-blue-600 p-6 flex items-center justify-between text-white flex-shrink-0 relative overflow-hidden">
                        ...
                        <h3 class="text-xl font-bold tracking-wide relative z-10 flex items-center gap-2"><Users class="w-5 h-5" /> Profil {{ hasDetailedInfo ? 'Lengkap' : 'Singkat' }}</h3>
                            ...
                            <div class="flex-shrink-0" :class="hasDetailedInfo ? 'lg:w-[32%] text-center' : 'w-full'">
                                ...
                                <div class="space-y-4 p-6 rounded-3xl bg-white border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]" :class="hasDetailedInfo ? 'text-left' : 'inline-block text-left w-full'">
                                    <div v-if="selectedPerson.structural_position" class="flex items-start gap-4">
                                        ...
                                        <div><p class="text-xs text-gray-400 font-semibold uppercase mb-0.5">Struktural</p><span class="text-sm font-bold text-gray-800 leading-tight">{{ selectedPerson.structural_position }}</span></div>
                                    </div>
                                    <div v-if="selectedPerson.functional_position" class="flex items-start gap-4">
                                        ...
                                        <div><p class="text-xs text-gray-400 font-semibold uppercase mb-0.5">Fungsional</p><span class="text-sm font-bold text-gray-800 leading-tight">{{ selectedPerson.functional_position }}</span></div>
                                    </div>
                                </div>

                                <div v-if="selectedPerson.academic_profiles?.length > 0" class="mt-8 flex flex-wrap justify-center gap-3">
                                    <a v-for="(link, i) in selectedPerson.academic_profiles" :key="i" :href="link" target="_blank" rel="noopener noreferrer" class="w-12 h-12 flex items-center justify-center bg-white border border-gray-200 text-gray-600 rounded-full hover:bg-primary hover:text-white hover:border-primary hover:shadow-lg hover:-translate-y-1 transition-all">
                            ...
                            <div v-if="hasDetailedInfo" class="lg:w-[68%] flex flex-col gap-10 lg:pl-10">
                                ...
                                <div v-if="selectedPerson.expertise?.length > 0" class="bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                                    ...
                                    <div class="flex flex-wrap gap-2.5">
                                        <span v-for="(expert, i) in selectedPerson.expertise" :key="i" class="px-5 py-2.5 bg-slate-50 text-slate-700 text-sm font-bold rounded-xl border border-slate-200 hover:border-primary hover:text-primary transition-colors cursor-default">{{ expert }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div v-if="selectedPerson.education_history?.length > 0">
                                        ...
                                        <ul class="space-y-4">
                                            <li v-for="(edu, i) in selectedPerson.education_history" :key="i" class="relative pl-6 before:absolute before:left-0 before:top-2 before:w-2 before:h-2 before:bg-primary before:rounded-full text-gray-700 font-medium">{{ edu }}</li>
                                        </ul>
                                    </div>
                                    <div v-if="selectedPerson.work_experience?.length > 0">
                                        ...
                                        <ul class="space-y-4">
                                            <li v-for="(work, i) in selectedPerson.work_experience" :key="i" class="relative pl-6 before:absolute before:left-0 before:top-2 before:w-2 before:h-2 before:bg-primary before:rounded-full text-gray-700 font-medium">{{ work }}</li>
                                        </ul>
                                    </div>
                                </div>
                                ...
                                <div v-if="selectedPerson.community_service_history?.length > 0" class="bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                                    <h4 class="flex items-center gap-3 text-lg font-bold text-gray-900 mb-5"><BookOpen class="w-6 h-6 text-primary" /> Pengabdian Masyarakat</h4>
                                    <ul class="space-y-4">
                                        <li v-for="(pkm, i) in selectedPerson.community_service_history" :key="i" class="flex items-start gap-4 text-gray-700 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                            <div class="mt-1 w-2.5 h-2.5 rounded-full bg-primary/50 ring-4 ring-primary/10 flex-shrink-0"></div>
                                            <span class="text-[15px] font-medium leading-relaxed">{{ pkm }}</span>
                                        </li>
                                    </ul>
                                </div>
