    ...
    <PublicLayout>
        ...
        <div class="bg-white py-16 md:py-24 overflow-x-hidden font-public-sans">
            <div class="container mx-auto px-4 sm:px-6 md:max-w-5xl flex flex-col gap-12">
                ...
                <div v-if="leaders.length === 0" class="text-center text-gray-500 py-10">
                    Belum ada data pimpinan jurusan.
                </div>

                <template v-for="(p, index) in leaders" :key="p.id">
                    <div
                        :data-aos="index % 2 === 0 ? 'fade-right' : 'fade-left'"
                        class="flex flex-col md:flex-row rounded-[2rem] md:rounded-[2.5rem] shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-500 group bg-white border border-gray-100"
                    ...
                    >
                        ...
                        <div class="w-full md:w-3/5 flex flex-col justify-center p-8 sm:p-10 md:p-14 lg:p-16 text-center md:text-left relative bg-gradient-to-br from-primary to-primary-hover text-white">
                            ...
                            <div class="inline-flex items-center justify-center md:justify-start gap-2 px-5 py-2 bg-white/10 text-white font-bold rounded-xl uppercase tracking-wider text-xs mb-5 md:mb-6 w-max mx-auto md:mx-0 border border-white/20 backdrop-blur-sm z-10 shadow-sm">
                                Ketua Jurusan
                            </div>
                            
                            <h3 class="font-optimus font-bold text-2xl sm:text-3xl md:text-4xl text-white mb-3 md:mb-4 leading-tight group-hover:text-blue-100 transition-colors duration-300 z-10 drop-shadow-md">
                                {{ p.name }}
                            </h3>
                            
                            <div class="w-16 h-1.5 bg-white/40 rounded-full mb-4 md:mb-6 opacity-80 mx-auto md:mx-0 z-10"></div>
                            
                            <p class="text-base sm:text-lg md:text-xl text-white/95 font-medium leading-relaxed tracking-wide z-10">
