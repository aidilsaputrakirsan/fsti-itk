...
<template>
    <PublicLayout>
        ...
        <div class="bg-white py-12 md:py-24 min-h-screen font-public-sans overflow-x-hidden">
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
                ...
                <div class="relative z-20 -mt-12 md:-mt-16 mx-2 sm:mx-4 md:mx-8 mb-12 md:mb-16 bg-white p-3 md:p-5 rounded-2xl shadow-[0_8px_30px_rgba(47,77,211,0.08)] border border-slate-100 flex flex-col md:flex-row gap-4" data-aos="fade-down">
                    <div class="relative flex-grow">
                        <input 
                            type="text" 
                            placeholder="Cari nama institusi atau mitra..." 
                            class="w-full pl-10 md:pl-12 pr-10 py-3 md:py-3.5 text-sm md:text-base border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary bg-slate-50 text-slate-800 font-medium hover:bg-white transition-colors"
                            v-model="searchQuery"
                        >
                        <Search class="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 w-4 md:w-5 h-4 md:h-5 text-primary/60" />
                        <button v-if="searchQuery" @click="searchQuery = ''" class="absolute right-2 md:right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-primary transition-colors">
                            <X class="w-4 md:w-5 h-4 md:h-5" />
                        </button>
                    </div>
                </div>

                <div v-if="partners.data.length === 0" class="text-center py-12 md:py-20 bg-slate-50/50 rounded-[2rem] border border-dashed border-slate-200 shadow-sm" data-aos="fade-up">
                    <div class="w-20 h-20 md:w-24 md:h-24 bg-white rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6 shadow-sm border border-slate-100">
                        <Handshake class="w-8 h-8 md:w-10 md:h-10 text-primary/40" />
                    </div>
                    <h3 class="text-xl md:text-2xl font-bold text-slate-800 mb-2 font-optimus">Mitra Tidak Ditemukan</h3>
                    <p class="text-sm md:text-base text-slate-500 max-w-md mx-auto px-4">Mungkin mitra kerjasama yang Anda cari belum terdaftar di direktori kami.</p>
                </div>

                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6 lg:gap-8">
                    <div v-for="(partner, index) in partners.data" :key="partner.id" class="bg-white rounded-[1rem] md:rounded-[1.5rem] border border-slate-200 shadow-sm hover:shadow-xl hover:border-primary/30 transform hover:-translate-y-1 transition-all duration-300 flex flex-col h-[300px] sm:h-[380px] md:h-[460px] group" data-aos="fade-up" :data-aos-delay="(index % 10) * 50">
                        
                        <div class="h-28 sm:h-36 md:h-52 w-full bg-white border-b border-slate-100 p-3 sm:p-4 md:p-6 flex items-center justify-center rounded-t-[1rem] md:rounded-t-[1.5rem] shrink-0">
                        ...
                        <div class="p-3 sm:p-4 md:p-6 flex flex-col flex-grow bg-slate-50/50 rounded-b-[1rem] md:rounded-b-[1.5rem]">
                            
                            <div v-if="partner.activities && partner.activities.length > 0" class="mb-2 md:mb-3">
                            ...
                            <h3 class="text-sm sm:text-base md:text-lg font-bold text-slate-800 group-hover:text-primary transition-colors leading-snug mb-2 md:mb-4 line-clamp-2">{{ partner.name }}</h3>
                            
                            <div v-if="partner.activities && partner.activities.length > 0" class="flex-grow overflow-y-auto pr-1 md:pr-2 space-y-2 md:space-y-3 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                                <ul class="space-y-1.5 md:space-y-3">
                                    <li v-for="(act, idx) in partner.activities" :key="idx" class="flex items-start gap-1.5 md:gap-2.5 text-[11px] sm:text-xs md:text-sm text-slate-600 font-medium">
                ...
                <div v-if="totalPages > 1" class="mt-12 md:mt-16 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6 bg-white py-4 px-4 md:px-10 rounded-3xl md:rounded-full shadow-sm border border-slate-100" data-aos="fade-in">
                    ...
                    <div class="flex flex-wrap justify-center items-center gap-1.5 md:gap-2">
                        <button @click="changePage(currentPage - 1)" :disabled="currentPage === 1" class="min-w-[2rem] md:min-w-[2.5rem] h-8 md:h-10 px-2 md:px-4 flex items-center justify-center text-xs md:text-sm font-bold rounded-full transition-all duration-300" :class="currentPage === 1 ? 'text-slate-300 bg-slate-50/50 cursor-not-allowed' : 'text-slate-600 hover:bg-slate-100 hover:text-primary'">&laquo; Sebelumnya</button>

                        <template v-for="page in visiblePages" :key="page">
                            <span v-if="page === '...'" class="min-w-[2rem] md:min-w-[2.5rem] h-8 md:h-10 px-2 md:px-4 flex items-center justify-center text-xs md:text-sm font-bold rounded-full text-slate-300 bg-slate-50/50 cursor-not-allowed">...</span>
                            <button v-else @click="changePage(page)" class="min-w-[2rem] md:min-w-[2.5rem] h-8 md:h-10 px-2 md:px-4 flex items-center justify-center text-xs md:text-sm font-bold rounded-full transition-all duration-300" :class="currentPage === page ? 'bg-primary text-white shadow-md' : 'text-slate-600 hover:bg-slate-100 hover:text-primary'">{{ page }}</button>
                        </template>

                        <button @click="changePage(currentPage + 1)" :disabled="currentPage === totalPages" class="min-w-[2rem] md:min-w-[2.5rem] h-8 md:h-10 px-2 md:px-4 flex items-center justify-center text-xs md:text-sm font-bold rounded-full transition-all duration-300" :class="currentPage === totalPages ? 'text-slate-300 bg-slate-50/50 cursor-not-allowed' : 'text-slate-600 hover:bg-slate-100 hover:text-primary'">Selanjutnya &raquo;</button>
                    </div>
                </div>
