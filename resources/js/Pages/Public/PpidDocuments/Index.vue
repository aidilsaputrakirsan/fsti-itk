...
const getIcon = (type) => {
    const icons = {
        ...
        'Setiap Saat': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 md:w-10 md:h-10"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 6.75h12M8.25 12h12m-12 5.25h12M3.75 6.75h.007v.008H3.75V6.75zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zM3.75 12h.007v.008H3.75V12zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm-.375 5.25h.007v.008H3.75v-.008zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" /></svg>',
        'Serta Merta': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 md:w-10 md:h-10"><path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0M3.124 7.5A8.969 8.969 0 015.292 3m13.416 0a8.969 8.969 0 012.168 4.5" /></svg>'
    };
    return icons[type] || icons['Berkala']; 
};
</script>
                ...
                <div class="flex flex-wrap justify-center gap-3 md:gap-8 max-w-6xl mx-auto px-1 md:px-4">
                    ...
                    >
                        ...
                        <div class="shrink-0 flex flex-col sm:flex-row items-center justify-center gap-1.5 md:gap-3 mt-3 md:mt-6 mb-4 md:mb-8">
                            ...
                            <span class="text-[8px] md:text-xs font-bold bg-primary/10 text-primary px-2 py-1 md:px-3 md:py-1.5 rounded-md whitespace-nowrap">
                                {{ info.total_documents }} Dokumen
                            </span>
                        </div>
        ...
        <section class="bg-white py-12 md:py-24 font-public-sans">
            <div class="container mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center mb-12 md:mb-16">
                    <h2 class="text-xl md:text-3xl font-bold font-optimus text-primary uppercase">Layanan & Tata Cara</h2>
                    <p class="mt-2 md:mt-3 text-xs md:text-base text-gray-600 font-medium">SOP Pengelolaan Informasi Publik dan Akses Formulir Layanan</p>
                </div>
                ...
                <div class="max-w-5xl mx-auto mb-16 md:mb-20">
                    <div class="flex flex-col md:flex-row justify-between items-center md:items-start relative gap-6 md:gap-0">
                        ...
                        <div class="md:hidden absolute top-0 left-1/2 -translate-x-1/2 w-0.5 h-full bg-primary/20 z-0"></div>
                        ...
                        <div 
                            ...
                            :class="{ 'mt-4 md:mt-0': index > 0 }"
                        >
                            <div class="w-12 h-12 md:w-16 md:h-16 rounded-full bg-primary text-white flex items-center justify-center text-lg md:text-2xl font-bold mb-4 md:mb-6 border-4 border-white shadow-md relative z-10">
                                {{ index + 1 }}
                            </div>
                            <h4 class="font-bold text-primary text-sm md:text-lg mb-2 md:mb-3 bg-white px-2">{{ alur.title }}</h4>
                            <p class="text-[10px] md:text-sm text-gray-600 leading-relaxed max-w-[250px] md:max-w-none bg-white px-2">{{ alur.description }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-2 gap-3 md:gap-8 max-w-4xl mx-auto">
                    ...
                    >
                        <div class="p-2 md:p-4 bg-blue-50 text-primary rounded-full mb-3 md:mb-5" v-html="layanan.icon"></div>
                        <h3 class="text-xs md:text-xl font-bold text-gray-900 mb-2 md:mb-3 leading-snug">{{ layanan.title }}</h3>
                        <p class="text-gray-500 text-[9px] md:text-sm mb-4 md:mb-8 flex-grow leading-relaxed line-clamp-3 md:line-clamp-none">{{ layanan.description }}</p>
                        <a 
                            :href="layanan.actionLink"
                            target="_blank"
                            class="w-full text-center py-2 md:py-3 px-3 md:px-6 bg-primary text-white hover:bg-primary-hover text-[10px] md:text-base font-semibold rounded-lg shadow-sm transition-colors mt-auto"
                        >
                            {{ layanan.actionText }}
                        </a>
                    </div>
