...
const form = useForm<TestimonialFormData>({
    ...
    job: props.testimonial.job || '',
    study_program: props.testimonial.study_program || '',
    graduation_year: props.testimonial.graduation_year || '',
...
<template>
    <div>
        <Head :title="'Edit Testimoni: ' + (testimonial.name || '')" />
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900">Edit Testimoni</h1>
            <p class="mt-1 text-gray-600">Perbarui data pengalaman dan kesan kelulusan alumni FSTI ITK.</p>
        </div>

        <div v-if="testimonial.id" class="bg-white shadow-sm p-5 sm:p-8 rounded-xl border-t-4 border-primary">
                ...
                <div class="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-x-8 gap-y-6 md:gap-y-8">
                    ...
                    <div>
                        <input v-model="form.name" type="text" 
                            ...
                            :class="form.errors.name ? 'border-red-500 bg-red-50' : 'border-gray-300 focus:border-primary bg-gray-50 focus:bg-white'" 
                            required>
                    ...
                    <div>
                        <div class="flex items-center gap-6 mb-4">
                            <img v-if="photo_url" :src="photo_url" alt="Foto Lama" class="w-24 h-24 rounded-full object-cover border-4 border-gray-100 shadow-sm">
                            <div v-else class="w-24 h-24 rounded-full bg-gray-100 border-4 border-white shadow-sm flex items-center justify-center">
                                <UserCircleIcon class="w-12 h-12 text-gray-400" />
                            </div>
                            <div class="text-sm text-gray-500">
                                <p class="font-bold text-gray-800 mb-1">Gambar saat ini</p>
                                <p>Setiap testimoni wajib memiliki foto wajah.<br>Biarkan kosong jika tidak ingin mengubah foto saat ini.</p>
                            </div>
                        </div>
                        ...
                        <input type="file" @change="handlePhotoUpload" accept="image/jpeg, image/png, image/jpg, image/webp" 
                            ...
                            :class="form.errors.photo ? 'border-red-500 bg-red-50' : 'border-gray-300 bg-gray-50'">
                        <p class="mt-1.5 text-xs text-gray-500">Format yang didukung: JPG, PNG, WEBP (Maks: 2MB).</p>
