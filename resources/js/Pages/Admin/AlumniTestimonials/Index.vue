...
import { 
    ...
    ExclamationTriangleIcon, CheckCircleIcon, UserCircleIcon 
...
watch(search, debounce(() => {
    router.get(route('admin.alumni-testimonials.index'), { 
        search: search.value,
    }, { preserveState: true, replace: true });
}, 300));

const formattedLinks = computed(() => {
    if (!props.testimonials?.links) return [];
    
    const links = props.testimonials.links.map((link: any) => {
...
const confirmDelete = () => {
    if (itemToDelete.value) {
        router.delete(route('admin.alumni-testimonials.destroy', itemToDelete.value.id), {
...
<template>
    <div>
        <Head title="Kelola Testimoni Alumni" />

        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Kelola Testimoni Alumni</h1>
                <p class="mt-1 text-gray-600">Manajemen pengalaman dan kesan kelulusan alumni FSTI ITK.</p>
            </div>
            <div class="flex items-center gap-3 flex-shrink-0">
                <Link :href="route('admin.alumni-testimonials.create')" class="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-primary-hover transition-colors flex-shrink-0">
        ...
        <div class="bg-white shadow-sm p-4 sm:p-6 rounded-xl border border-gray-100 overflow-hidden">
            <h3 class="text-lg font-bold text-gray-900 mb-4 hidden sm:block">Daftar Testimoni</h3>
                ...
                <table class="w-full min-w-[800px]">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="w-24 text-center">Foto</th>
                    ...
                    <tbody>
                        <tr v-if="testimonials.data && testimonials.data.length > 0" v-for="(item, index) in testimonials.data" :key="item.id">
                            <td class="text-center font-medium text-gray-500">
                                {{ (Number(testimonials.current_page) - 1) * Number(testimonials.per_page) + Number(index) + 1 }}
                            </td>
                            <td class="text-center">
                                <img v-if="item.photo_url" :src="item.photo_url" class="w-10 h-10 rounded-full object-cover border border-gray-200 shadow-sm mx-auto">
                                <div v-else class="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center border border-gray-200 mx-auto">
                            ...
                            <td class="text-center">
                                <span :class="item.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'" class="rounded-full px-3 py-1 text-xs font-bold">
                                    {{ item.is_active ? 'Aktif' : 'Disembunyikan' }}
                                </span>
                            </td>
                            <td>
                                <div class="flex items-center justify-center gap-3">
                                    <Link :href="route('admin.alumni-testimonials.edit', item.id)" class="flex items-center gap-1 text-primary hover:text-primary-hover font-semibold transition-colors">
                        ...
                        <tr v-else>
                            <td colspan="6" class="py-8 text-center text-gray-500 font-medium">Belum ada data testimoni alumni.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
                <p v-if="testimonials.total > 0" class="text-sm text-gray-600 text-center sm:text-left">
                    Menampilkan <span class="font-bold text-gray-900">{{ testimonials.from }}</span> sampai <span class="font-bold text-gray-900">{{ testimonials.to }}</span> dari <span class="font-bold text-gray-900">{{ testimonials.total }}</span> hasil
                </p>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm transition-opacity" @click.self="closeDeleteModal">
      <div class="w-full max-w-md rounded-2xl bg-white p-6 sm:p-8 shadow-2xl transform transition-all scale-100">
        <div class="flex flex-col items-center text-center">
          ...
          <h2 class="text-2xl font-bold text-gray-900">Hapus Testimoni?</h2>
          <p class="mt-2 text-gray-600 text-center">Apakah Anda yakin ingin menghapus data testimoni milik <br> <span class="font-bold text-gray-900">"{{ itemToDelete?.name }}"</span>?</p>
        </div>
