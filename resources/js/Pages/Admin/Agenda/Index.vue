...
watch([search, year], debounce(() => {
    router.get((route as Function)('admin.agenda-fakultas.index'), { 
        search: search.value,
...
const formattedLinks = computed(() => {
    if (!props.agendas?.links) return [];
    
    const links = props.agendas.links.map((link: any) => {
...
const confirmDelete = () => {
    if (itemToDelete.value) {
        router.delete((route as Function)('admin.agenda-fakultas.destroy', itemToDelete.value.id), {
...
<template>
    <div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Kelola Agenda Fakultas</h1>
                <p class="mt-1 text-gray-600">Manajemen jadwal resmi, akademik, dan institusi FSTI ITK.</p>
            </div>
            <Link :href="(route as Function)('admin.agenda-fakultas.create')" class="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-primary-hover transition-colors flex-shrink-0">
        ...
        <div class="bg-white shadow-sm p-4 sm:p-6 rounded-xl border border-gray-100 overflow-hidden">
            <h3 class="text-lg font-bold text-gray-900 mb-4 hidden sm:block">Daftar Agenda</h3>
            ...
            <div class="admin-table-container overflow-x-auto w-full">
                <table class="w-full min-w-[800px]">
                    ...
                    <tbody>
                        <tr v-if="agendas.data && agendas.data.length > 0" v-for="item in agendas.data" :key="item.id">
                            <td>
                            ...
                            <td>
                                <div class="flex items-center justify-center gap-3">
                                    <Link :href="(route as Function)('admin.agenda-fakultas.edit', item.id)" class="flex items-center gap-1 text-primary hover:text-primary-hover font-semibold transition-colors">
                        ...
                        <tr v-else>
                            <td colspan="4" class="py-8 text-center text-gray-500 font-medium">Tidak ada agenda yang ditemukan.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
                <p v-if="agendas.total > 0" class="text-sm text-gray-600 text-center sm:text-left">
                    Menampilkan <span class="font-bold text-gray-900">{{ agendas.from }}</span> sampai <span class="font-bold text-gray-900">{{ agendas.to }}</span> dari <span class="font-bold text-gray-900">{{ agendas.total }}</span> hasil
                </p>
                <p v-else></p>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm transition-opacity" @click.self="closeDeleteModal">
        <div class="w-full max-w-md rounded-2xl bg-white p-6 sm:p-8 shadow-2xl transform transition-all scale-100">
            <div class="flex flex-col items-center text-center">
                ...
                <h2 class="text-2xl font-bold text-gray-900">Hapus Agenda?</h2>
