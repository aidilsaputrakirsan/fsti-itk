...
const form = useForm<KategoriFormData>({
    _method: 'PUT',
    jenis_informasi: props.category.jenis_informasi || '',
    nama_kategori: props.category.nama_kategori || '',
    urutan: props.category.urutan || 0,
});
...
const submit = () => {
    ...
    form.post(route('admin.kategori-ppid.update', props.category.id));
};
</script>

<template>
    <div>
        <Head :title="'Edit Kategori PPID: ' + (category.nama_kategori || '')" />

        <div class="mb-8">
            <Link :href="route('admin.kategori-ppid.index')" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-white border border-gray-300 text-gray-700 font-bold hover:bg-gray-50 transition-colors shadow-sm w-fit mb-6">
        ...
        <div v-if="category.id" class="bg-white shadow-sm p-5 sm:p-8 rounded-xl border-t-4 border-primary">
            <form @submit.prevent="submit" novalidate>
                ...
                <div class="mt-12 flex flex-col-reverse sm:flex-row items-center justify-end gap-3 border-t border-gray-100 pt-6">
                    <Link :href="route('admin.kategori-ppid.index')" class="w-full sm:w-auto text-center rounded-lg border border-gray-300 bg-white px-6 py-2.5 text-sm font-bold text-gray-700 shadow-sm hover:bg-gray-50 transition-colors">
                        Batal
                    </Link>
