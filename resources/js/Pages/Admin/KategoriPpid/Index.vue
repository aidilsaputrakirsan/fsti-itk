...
watch(search, debounce((value: string) => {
    router.get((route as Function)('admin.kategori-ppid.index'), { 
        search: value 
    }, { 
        preserveState: true, 
        replace: true 
...
const formattedLinks = computed(() => {
    if (!props.kategoris?.links) return [];
    
    const links = props.kategoris.links.map((link: any) => {
...
const confirmDelete = () => {
    if (itemToDelete.value) {
        router.delete((route as Function)('admin.kategori-ppid.destroy', itemToDelete.value.id), {
...
<template>
    <div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <Link :href="(route as Function)('admin.kategori-ppid.create')" class="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-primary-hover transition-colors flex-shrink-0">
        ...
        <div class="bg-white shadow-sm p-4 sm:p-6 rounded-xl border border-gray-100 overflow-hidden">
            ...
            <div class="admin-table-container overflow-x-auto w-full">
                <table class="w-full min-w-[800px]">
                    ...
                    <tbody>
                        <tr v-if="kategoris.data && kategoris.data.length > 0" v-for="(item, index) in kategoris.data" :key="item.id">
                            <td class="text-center font-medium text-gray-500">
                                {{ (Number(kategoris.current_page) - 1) * Number(kategoris.per_page) + Number(index) + 1 }}
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="flex items-center justify-center gap-3">
                                    <Link :href="(route as Function)('admin.kategori-ppid.edit', item.id)" class="flex items-center gap-1 text-primary hover:text-primary-hover font-semibold transition-colors">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
                <p v-if="kategoris.total > 0" class="text-sm text-gray-600 text-center sm:text-left">
                    Menampilkan <span class="font-bold text-gray-900">{{ kategoris.from }}</span> sampai <span class="font-bold text-gray-900">{{ kategoris.to }}</span> dari <span class="font-bold text-gray-900">{{ kategoris.total }}</span> hasil
                </p>
                <p v-else></p>
