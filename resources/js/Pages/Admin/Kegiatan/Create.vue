...
const submit = () => {
    form.post(route('admin.kegiatan-mahasiswa.store'));
};
</script>
...
<template>
    ...
    <div>
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-black">Tambah Kegiatan</h1>
            <p class="mt-1 text-black">Tambahkan data kegiatan mahasiswa baru</p>
        </div>
        ...
        <div class="bg-white shadow-sm p-8 rounded-lg">
            <form @submit.prevent="submit">
                <div class="grid grid-cols-1 md:grid-cols-[150px_1fr] gap-x-8 gap-y-8">
                    ...
                    <div>
                        <input v-model="form.title" type="text" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Masukkan judul kegiatan">
                        <p v-if="form.errors.title" class="mt-2 text-sm text-red-600">{{ form.errors.title }}</p>
                    </div>

                    <label class="pt-2 text-sm font-semibold text-black">Penyelenggara</label>
                    <div>
                        <input v-model="form.organizer" type="text" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Contoh: HIMA Statistika, BEM FSTI">
                    ...
                    <div>
                        <input v-model="form.location" type="text" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Contoh: Auditorium Kampus">
                    ...
                    <div>
                        <textarea v-model="form.description" rows="4" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Keterangan singkat mengenai kegiatan..."></textarea>
                ...
                <div class="mt-12 flex items-center justify-between">
                    ...
                    <button type="submit" :disabled="form.processing" class="flex items-center gap-2 rounded-lg bg-[#4682A9] px-6 py-2 text-sm font-semibold text-white shadow-sm hover:bg-opacity-90 disabled:opacity-50">
                        <CheckIcon class="h-5 w-5" /> Simpan Kegiatan
                    </button>
                </div>
            </form>
        </div>
