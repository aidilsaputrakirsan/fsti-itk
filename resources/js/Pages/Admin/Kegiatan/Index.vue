...
const props = defineProps({
    kegiatan: Object,
    filters: Object,
    availableYears: Array, // <-- Menerima data tahun dari Controller
});

const search = ref(props.filters.search || '');
const year = ref(props.filters.year || ''); // <-- State untuk tahun

// Watcher untuk Search dan Filter Tahun
...
<template>
    ...
    <div>
        <div class="flex items-start justify-between mb-8">
            <div>
                <h1 class="text-3xl font-bold text-black">Kelola Kegiatan Mahasiswa</h1>
                <p class="mt-1 text-black">Manajemen daftar agenda dan aktivitas kemahasiswaan FSTI ITK</p>
            </div>
            <Link :href="route('admin.kegiatan-mahasiswa.create')" class="flex items-center gap-2 rounded-lg bg-[#4682A9] px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-opacity-90 flex-shrink-0">
        ...
        <div class="bg-white shadow-sm p-6 rounded-lg">
            <h3 class="text-lg font-semibold text-black mb-4">Daftar Kegiatan</h3>
            
            <div class="border rounded-lg overflow-x-auto">
                <table class="w-full min-w-full">
                    <thead class="bg-[#CBDCEB]">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-black">Judul Kegiatan</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <tr v-if="kegiatan.data.length > 0" v-for="item in kegiatan.data" :key="item.id" class="hover:bg-gray-50">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                <div class="flex items-center gap-2">
                                    <Link :href="route('admin.kegiatan-mahasiswa.edit', item.id)" class="flex items-center gap-1 text-[#4682A9] hover:opacity-80">
                        ...
                        <tr v-else>
                            <td colspan="4" class="text-center py-4 text-gray-500">Tidak ada kegiatan yang ditemukan.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="flex items-center justify-between mt-4">
                <p v-if="kegiatan.total > 0" class="text-sm text-black">
                    Menampilkan <span class="font-medium">{{ kegiatan.from }}</span> sampai <span class="font-medium">{{ kegiatan.to }}</span> dari <span class="font-medium">{{ kegiatan.total }}</span> hasil
                </p>
                <p v-else></p>

                <div class="flex items-center gap-1">
                    <Link 
                        v-for="(link, index) in kegiatan.links" :key="index" :href="link.url ?? '#'" v-html="link.label"
                        :class="[
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="w-full max-w-md rounded-lg bg-white p-8 shadow-2xl">
            <div class="flex flex-col items-center text-center">
                ...
                <h2 class="text-2xl font-bold text-gray-800">Hapus Kegiatan</h2>
