...
watch([search, year], debounce(() => {
    router.get((route as Function)('admin.kegiatan-mahasiswa.index'), { 
        search: search.value,
...
const formattedLinks = computed(() => {
    if (!props.kegiatan?.links) return [];
    
    const links = props.kegiatan.links.map((link: any) => {
...
const confirmDelete = () => {
    if (itemToDelete.value) {
        router.delete((route as Function)('admin.kegiatan-mahasiswa.destroy', itemToDelete.value.id), {
...
<template>
    <div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <Link :href="(route as Function)('admin.kegiatan-mahasiswa.create')" class="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-primary-hover transition-colors flex-shrink-0">
        ...
        <div class="bg-white shadow-sm p-4 sm:p-6 rounded-xl border border-gray-100 overflow-hidden">
            ...
            <div class="admin-table-container overflow-x-auto w-full">
                <table class="w-full min-w-[800px]">
                    ...
                    <tbody>
                        <tr v-if="kegiatan.data && kegiatan.data.length > 0" v-for="item in kegiatan.data" :key="item.id">
                            <td>
                            ...
                            <td>
                                <div class="flex items-center justify-center gap-3">
                                    <Link :href="(route as Function)('admin.kegiatan-mahasiswa.edit', item.id)" class="flex items-center gap-1 text-primary hover:text-primary-hover font-semibold transition-colors">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
                <p v-if="kegiatan.total > 0" class="text-sm text-gray-600 text-center sm:text-left">
                    Menampilkan <span class="font-bold text-gray-900">{{ kegiatan.from }}</span> sampai <span class="font-bold text-gray-900">{{ kegiatan.to }}</span> dari <span class="font-bold text-gray-900">{{ kegiatan.total }}</span> hasil
                </p>
                <p v-else></p>
