...
const formattedLinks = computed(() => {
    if (!props.scholarships?.links) return [];
    
    const links = props.scholarships.links.map((link: any) => {
...
<template>
    <div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <Link :href="route('admin.beasiswa.create')" class="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-primary-hover transition-colors flex-shrink-0">
        ...
        <div class="bg-white shadow-sm p-4 sm:p-6 rounded-xl border border-gray-100 overflow-hidden">
            ...
            <div class="admin-table-container overflow-x-auto w-full">
                <table class="w-full min-w-[800px]">
                    ...
                    <tbody>
                        <tr v-if="scholarships.data && scholarships.data.length > 0" v-for="(item, index) in scholarships.data" :key="item.id">
                            <td class="text-center font-medium text-gray-500">
                                {{ (Number(scholarships.current_page) - 1) * Number(scholarships.per_page) + Number(index) + 1 }}
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="flex items-center justify-center gap-3">
                                    <Link :href="route('admin.beasiswa.edit', item.id)" class="flex items-center gap-1 text-primary hover:text-primary-hover font-semibold transition-colors">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
                <p v-if="scholarships.total > 0" class="text-sm text-gray-600 text-center sm:text-left">
                    Menampilkan <span class="font-bold text-gray-900">{{ scholarships.from }}</span> sampai <span class="font-bold text-gray-900">{{ scholarships.to }}</span> dari <span class="font-bold text-gray-900">{{ scholarships.total }}</span> hasil
                </p>
                <p v-else></p>
