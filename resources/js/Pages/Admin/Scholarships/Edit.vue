...
const form = useForm<BeasiswaFormData>({
    ...
    link_url: props.scholarship.link_url || '',
...
const submit = () => {
    ...
    const targetUrl: string = route('admin.beasiswa.update', props.scholarship.id);
    form.post(targetUrl);
};
</script>
...
<template>
    <div>
        <Head :title="'Edit Beasiswa: ' + (scholarship.title || '')" />
        ...
        <div v-if="scholarship.id" class="bg-white shadow-sm p-5 sm:p-8 rounded-xl border-t-4 border-primary">
