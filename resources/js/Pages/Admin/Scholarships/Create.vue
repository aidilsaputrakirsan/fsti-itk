...
const form = useForm<BeasiswaForm>({
    title: '',
    provider: '',
    link_url: '',
    description: '',
...
<template>
    <div>
        ...
        <div class="mb-8">
            <Link :href="route('admin.beasiswa.index')" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-white border border-gray-300 text-gray-700 font-bold hover:bg-gray-50 transition-colors shadow-sm w-fit mb-6">
                <ArrowLeftIcon class="h-4 w-4 stroke-2" /> Kembali ke Daftar
            </Link>
            <h1 class="text-3xl font-bold text-gray-900">Tambah Beasiswa</h1>
            <p class="mt-1 text-gray-600">Tambahkan data beasiswa dan bantuan pendidikan baru.</p>
        </div>
        ...
        <div class="bg-white shadow-sm p-5 sm:p-8 rounded-xl border-t-4 border-primary">
            <form @submit.prevent="submit" novalidate>
                <div class="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-x-8 gap-y-6 md:gap-y-8">
                    ...
                    <div>
                        <input v-model="form.title" type="text" placeholder="Contoh: Beasiswa Kaltim Tuntas" 
                    ...
                    <div>
                        <input v-model="form.provider" type="text" placeholder="Contoh: Pemprov Kaltim, Bank Indonesia" 
                            class="block w-full rounded-lg transition-colors py-3"
                    ...
                    <div>
                        <input v-model="form.link_url" type="url" placeholder="Contoh: https://beasiswa.kaltimprov.go.id" 
                            class="block w-full rounded-lg transition-colors py-3"
                    ...
                    <div>
                        <textarea v-model="form.description" rows="6" placeholder="Tuliskan keterangan, syarat, atau ketentuan beasiswa..." 
                            class="block w-full rounded-lg transition-colors py-3"
                ...
                <div class="mt-12 flex flex-col-reverse sm:flex-row items-center justify-end gap-3 border-t border-gray-100 pt-6">
                    <Link :href="route('admin.beasiswa.index')" class="w-full sm:w-auto text-center rounded-lg border border-gray-300 bg-white px-6 py-2.5 text-sm font-bold text-gray-700 shadow-sm hover:bg-gray-50 transition-colors">
                        Batal
                    </Link>
