...
const dataKegiatan = props.activity || props.data || {};
...
const form = useForm<KegiatanFormData>({
    _method: 'PUT',
    title: dataKegiatan.title || '',
    organizer: dataKegiatan.organizer || '',
    start_date: formatDateForInput(dataKegiatan.start_date),
    end_date: formatDateForInput(dataKegiatan.end_date),
    location: dataKegiatan.location || '',
    description: dataKegiatan.description || '',
});
...
const submit = () => {
    ...
    const targetUrl: string = route('admin.kegiatan-mahasiswa.update', dataKegiatan.id);
    form.post(targetUrl);
};
</script>
...
<template>
    <div>
        <Head :title="'Edit Kegiatan: ' + (dataKegiatan.title || '')" />

        <div class="mb-8">
            <Link :href="route('admin.kegiatan-mahasiswa.index')" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-white border border-gray-300 text-gray-700 font-bold hover:bg-gray-50 transition-colors shadow-sm w-fit mb-6">
                <ArrowLeftIcon class="h-4 w-4 stroke-2" /> Kembali ke Daftar
            </Link>
            <h1 class="text-3xl font-bold text-gray-900">Edit Kegiatan</h1>
            <p class="mt-1 text-gray-600">Perbarui data detail kegiatan mahasiswa FSTI ITK.</p>
        </div>

        <div v-if="dataKegiatan.id" class="bg-white shadow-sm p-5 sm:p-8 rounded-xl border-t-4 border-primary">
            <form @submit.prevent="submit" novalidate>
                ...
                <div class="mt-12 flex flex-col-reverse sm:flex-row items-center justify-end gap-3 border-t border-gray-100 pt-6">
                    <Link :href="route('admin.kegiatan-mahasiswa.index')" class="w-full sm:w-auto text-center rounded-lg border border-gray-300 bg-white px-6 py-2.5 text-sm font-bold text-gray-700 shadow-sm hover:bg-gray-50 transition-colors">
                        Batal
                    </Link>
