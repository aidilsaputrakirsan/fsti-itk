...
const submit = () => {
    ...
    const targetUrl: string = (route as Function)('admin.ppid.update', dataDocument.id);
    form.post(targetUrl);
};
</script>
...
<template>
    <div>
        <Head :title="'Edit Dokumen: ' + (dataDocument.judul_dokumen || '')" />
        ...
        <div class="mb-8">
            ...
            <p class="mt-1 text-gray-600">Perbarui informasi, file, atau tautan dokumen informasi publik.</p>
        </div>
        ...
        <div v-if="dataDocument.id" class="bg-white shadow-sm p-5 sm:p-8 rounded-xl border-t-4 border-primary">
            <form @submit.prevent="submit" novalidate>
                <div class="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-x-8 gap-y-6 md:gap-y-8">
                    ...
                    <div>
                        <input v-model="form.judul_dokumen" type="text" 
                            class="block w-full rounded-lg transition-colors py-3"
                    ...
                    <div>
                        ...
                        <div v-if="dataDocument.file_url && dataDocument.file_url.includes('/storage/') && !form.file" class="inline-flex items-center gap-1.5 text-xs font-bold text-green-700 bg-green-100 px-3 py-1.5 rounded-lg border border-green-200">
